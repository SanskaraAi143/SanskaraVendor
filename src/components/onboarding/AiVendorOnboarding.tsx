...
export const VendorOnboarding: React.FC<AiVendorOnboardingProps> = ({ onBack, onComplete, onError }) => {
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      setIsRecording(true);
      ws.current.send(JSON.stringify({ type: 'start_user_turn', userType: 'vendor' }));
    } catch (error) {
  ...
  const handleSendMessage = useCallback(() => {
    ...
    setAiStatus('Thinking...');
    ws.current.send(JSON.stringify({ type: 'text', content: inputMessage, userType: 'vendor' }));
    setInputMessage('');
  ...
  const handleSubmit = async (data: any) => {
    try {
      ...
      const vendorData = {
        vendor_name: data.venueName || '',
        vendor_category: 'Venue',
        contact_email: data.emailAddress || user?.email || '',
        phone_number: data.directPhoneNumbers || '',
        website_url: data.websiteLinks || null,
        address: {
        ...
        pricing_range: {
          min: parseInt(data.pricing?.vegStandard?.min || '0'),
          max: parseInt(data.pricing?.nonVegDeluxe?.max || '0'),
          currency: 'INR'
        },
  ...
  useEffect(() => {
    if (collectedData.venueName && collectedData.emailAddress && user) {
