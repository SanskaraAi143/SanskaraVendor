...
const Calendar: React.FC = () => {
  ...
  const fetchCalendarData = async () => {
    ...
    try {
      ...
      const enhancedBookings = await Promise.all(
        bookingsSnapshot.docs.map(async (bookingDoc) => {
          ...
          if (bookingData.user_id) {
            ...
            if (userSnap.exists()) {
              client_name = userSnap.data().display_name || 'Unknown Client';
            }
          }
      ...
      const availData = availSnapshot.docs.map(doc => ({
        availability_id: doc.id,
        ...doc.data()
      })) as AvailabilityInfo[];
  ...
  const setAvailabilityStatus = async (status: string) => {
    ...
    try {
      ...
      const availRef = collection(db, 'vendor_availability');
      const q = query(
        availRef,
        where('vendor_id', '==', vendorProfile.vendor_id),
      ...
      if (!querySnapshot.empty) {
        ...
        const docRef = doc(db, 'vendor_availability', docId);
      ...
      newAvailabilityMap[formattedDate] = status;
  ...
  const handleAddAvailability = async () => {
    ...
    try {
      await addDoc(collection(db, 'vendor_availability'), {
        vendor_id: vendorProfile.vendor_id,
